...
def exists(url):
    if url.status_code >= 200 and url.status_code < 400:
        return True
    else:
        return False
...
def imgExactGet(textr, callback):
    add = 0
    url = requests.get(f'https://gimn25.eduface.ru/uploads/62400/62391/section/2192375/{current(0, 0, textr)}_{current(2, 0, textr)}_2025_goda__{current(1, 0, textr)}_.pdf', stream=True)
    if exists(url):
        img = convert_from_bytes(url.raw.read(), poppler_path=pathP)
...
def imgGet(add, callback):
    url = requests.get(f'https://gimn25.eduface.ru/uploads/62400/62391/section/2192375/{current(0, add, 0)}_{current(2, add, 0)}_2025_goda__{current(1, add, 0)}_.pdf', stream=True, timeout=10)
    if exists(url):
        t = datetime.datetime.now()
        tt = t + datetime.timedelta(days=add)
        img = convert_from_bytes(url.raw.read(), poppler_path=pathP)
...
def imgDel(add):
    url = requests.get(f'https://gimn25.eduface.ru/uploads/62400/62391/section/2192375/{current(0, add, 0)}_{current(2, add, 0)}_2025_goda__{current(1, add, 0)}_.pdf', stream=True)
    if exists(url):
        img = convert_from_bytes(url.raw.read(), poppler_path=pathP)
        img_name=f"img-{current(0, add, 0)}-{current(2, add, 0)}-{current(1, add, 0)}.jpeg"
        os.remove(f"./temp/{img_name}")
